...
async function checkBackendStatus() {
  ...
  try {
      ...
      try {
        ...
        if (statusResponse.ok) {
          ...
          if (statusData.data?.clusters?.length > 0) {
            console.log(
              'Cluster names:',
              statusData.data.clusters.map(c => c.name).join(', ')
            );
          } else {
            console.log(
              '⚠️ No clusters found. Make sure you have Kind clusters running.'
            );
          }
        } else {
          console.log(
            '❌ Cluster status endpoint returned error:',
            statusResponse.status
          );
        }
      } catch (statusError) {
        console.log(
          '❌ Error accessing cluster status endpoint:',
          statusError.message
        );
      }
    } else {
    ...
    console.log(
      '2. Check if the server is running on the correct port (default: 8020)'
    );
