...
export default function ClusterManagementForm() {
  ...
  const handleCreateCluster = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    setError(null)
    setIsLoading(true)
    setProgress(0)
    ...
    try {
      ...
      await clusterApi.createCluster(`kind-cluster-${environment}`, environment, parseInt(workerNodes))
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while creating the cluster')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleDeployApplication = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while deploying the application')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'An error occurred while checking cluster status')
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex space-x-4 mb-6">
        <Button variant="secondary" onClick={handleCreateCluster} disabled={isLoading}>
          {isLoading ? 'Creating...' : 'Create Cluster'}
        </Button>
        <Button variant="ghost" onClick={handleDeployApplication} disabled={isLoading}>
          {isLoading ? 'Deploying...' : 'Deploy Application'}
        </Button>
        <Button variant="ghost" onClick={handleCheckStatus} disabled={isLoading}>
          Check Status
        </Button>
      </div>
      <h2 className="text-2xl font-semibold mb-4">Create a New Cluster</h2>
      <p className="text-muted-foreground mb-6">Set up a new Kind cluster for your environment</p>
      <form className="space-y-6">
        ...
        <div>
          <label htmlFor="worker-nodes" className="block text-sm font-medium mb-2">
            Worker Nodes
          </label>
          <Input 
            id="worker-nodes" 
