      ...
      case 'spinner':
        return (
          <div className={`${containerClass} ${className}`}>
            ...
            {text && <p className={`mt-3 ${sizeMap[size].text} text-muted-foreground`}>{text}</p>}
          </div>
        );
          ...
          <div className={`${containerClass} w-full max-w-md ${className}`}>
            <div className="w-full">
              <div className="flex justify-between mb-2">
                <span className={`${sizeMap[size].text} text-muted-foreground`}>{text || 'Loading...'}</span>
                <span className={`${sizeMap[size].text} ${colorClass}`}>{`${Math.round(progress)}%`}</span>
              </div>
              <div className={`w-full bg-muted rounded-full overflow-hidden ${sizeMap[size].bar}`}>
                <motion.div
      ...
      case 'kubernetes':
        return (
          <div className={`${containerClass} ${className}`}>
            ...
            {text && <p className={`mt-5 ${sizeMap[size].text} text-muted-foreground`}>{text}</p>}
          </div>
        );
