      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        {segments.map((segment, index) => (
          <motion.path
            ...
            onClick={() => interactive && onItemClick && onItemClick(segment.item, index)}
            style={{ 
          ...
          {segments.map((segment, index) => {
            ...
            return (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ duration: 0.5, delay: animationDuration + (index * 0.1) }}
              >
                <div className="flex items-center gap-1">
                  {!isRightSide && (
                    <>
                      {showValues && (
                        <span className="font-medium">{segment.item.value}</span>
                      )}
                      {showPercentages && (
