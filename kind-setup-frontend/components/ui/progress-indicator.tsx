            ...
            <React.Fragment key={step.id}>
              ...
              <div className="flex flex-col items-center">
                {showIcons ? (
                  ...
                  <div className={`w-4 h-4 rounded-full ${getStatusClasses(step.status, index)}`} />
                )}
                
                {showLabels && (
                  <div className="mt-2 text-center">
                    <p className={`text-sm font-medium ${step.status === 'in-progress' ? 'text-primary' : ''}`}>
                      {step.label}
                    </p>
                    {step.description && (
                ...
                <div className="flex-1 mx-2">
                  <div className="h-1 relative">
                    ...
                    <motion.div
                      ...
                      initial={{ scaleX: 0 }}
                      animate={{ 
                        scaleX: index < currentStepIndex ? 1 : 
                                index === currentStepIndex ? 0.5 : 0 
                      }}
          ...
          <div key={step.id} className="flex items-start mb-8 last:mb-0">
            ...
            <div className="flex flex-col items-center mr-4">
              {showIcons ? (
                ...
                <div className={`w-4 h-4 rounded-full z-10 ${getStatusClasses(step.status, index)}`} />
              )}
              ...
              {index < steps.length - 1 && (
                <div className="w-1 flex-1 my-2 relative h-full min-h-[40px]">
                  ...
                  <motion.div
                    ...
                    initial={{ scaleY: 0 }}
                    animate={{ 
                      scaleY: index < currentStepIndex ? 1 : 
                              index === currentStepIndex ? 0.5 : 0 
                    }}
                    transition={{ duration: 0.5, ease: "easeInOut" }}
                  ></motion.div>
