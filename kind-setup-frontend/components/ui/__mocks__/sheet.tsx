...
export const Sheet = ({ children, open }: { children: React.ReactNode, open?: boolean }) => {
  return <div data-testid="sheet" data-state={open ? "open" : "closed"}>{children}</div>;
};
...
export const SheetContent = ({ children, side }: { children: React.ReactNode, side?: string }) => {
  return <div data-testid="sheet-content" data-side={side}>{children}</div>;
};
