...
export function ClusterDetailModal({
  ...
  const [clusterHealth, setClusterHealth] = useState<ClusterHealth | null>(
    null
  );
  const [clusterResources, setClusterResources] =
    useState<ClusterResources | null>(null);
  const [clusterNetwork, setClusterNetwork] = useState<ClusterNetwork | null>(
    null
  );
  const [clusterStorage, setClusterStorage] = useState<ClusterStorage | null>(
    null
  );
  const [clusterWorkloads, setClusterWorkloads] =
    useState<ClusterWorkloads | null>(null);
  const [clusterEvents, setClusterEvents] = useState<ClusterEvents | null>(
    null
  );
                    ...
                    <CardContent>
                      <div className='space-y-2'>
                        <div className='flex items-center justify-between'>
                          ...
                          <span className='text-sm font-medium'>
                            {clusterDetails?.version?.client_version ||
                              'Unknown'}
                          </span>
                        </div>
                        <div className='flex items-center justify-between'>
                          ...
                          <span className='text-sm font-medium'>
                            {clusterDetails?.version?.server_version ||
                              'Unknown'}
                          </span>
                        </div>
                        <div className='flex items-center justify-between'>
