...
import { Server, Plus, AppWindow, Cpu, HardDrive, Settings, Book, X, ChevronRight } from 'lucide-react';
...
export const navSections = [
  ...
  },
  {
    ...
    items: [
      { href: '/create-cluster', label: 'Create Cluster', icon: <Plus className="h-4 w-4" /> },
      { href: '/cluster-status', label: 'Cluster Status', icon: <Cpu className="h-4 w-4" /> },
    ]
  },
  {
    title: "Applications",
    items: [
      { href: '/deploy-app', label: 'Deploy App', icon: <AppWindow className="h-4 w-4" /> },
      { href: '/manage-apps', label: 'Manage Apps', icon: <AppWindow className="h-4 w-4" /> },
    ]
  },
  {
    title: "Resources",
    items: [
      { href: '/manage-storage', label: 'Manage Storage', icon: <HardDrive className="h-4 w-4" /> },
      ...
      >
        ...
        {isActive ? (
          ...
          <motion.div
            ...
            initial={{ opacity: 0 }}
            whileHover={{ opacity: 0.5, backgroundColor: "hsl(var(--primary) / 0.3)" }}
            transition={{ duration: 0.2 }}
          />
        )}
      </Link>
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className="hidden md:flex flex-col w-64 h-screen border-r border-border bg-card/50 fixed left-0 top-16 shadow-sm"
      >
