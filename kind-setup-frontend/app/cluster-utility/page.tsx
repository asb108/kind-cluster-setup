...
export default function ClusterUtility() {
  ...
  const deleteCluster = async (clusterName: string) => {
    if (
      !confirm(`Are you sure you want to delete the cluster '${clusterName}'?`)
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8020'}/api/cluster/delete?name=${clusterName}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
