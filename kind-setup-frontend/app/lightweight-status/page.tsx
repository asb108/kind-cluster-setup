  ...
  const fetchDirectClusterStatus = async () => {
    ...
    try {
      ...
      if (!loading) {
        toast({
          title: "Error",
          description: "Failed to fetch clusters. The backend may be busy with cluster creation.",
          variant: "destructive",
        });
      }
    } finally {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">View running Kind clusters with minimal backend load</p>
        </div>
          ...
          {clusters.map((cluster) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="font-medium text-foreground">{cluster.status || "Unknown"}</div>
                
                <div className="text-muted-foreground">Nodes:</div>
                <div className="font-medium text-foreground">{cluster.nodes || "Unknown"}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-8 text-sm text-muted-foreground">
        <p>This page uses a lightweight approach to check cluster status that minimizes backend load.</p>
        <p>Use this page during cluster creation operations when the main dashboard might be slow to respond.</p>
      </div>
    </div>
  );
}
