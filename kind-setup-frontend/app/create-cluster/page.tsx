...
export default function CreateCluster() {
  ...
  const [creationPhase, setCreationPhase] = useState<'idle' | 'starting' | 'creating' | 'polling' | 'verifying' | 'completed' | 'failed'>('idle');
  ...
  useEffect(() => {
    ...
    if (urlTaskId && urlClusterName) {
      console.log(`🔄 Resuming task polling from URL: taskId=${urlTaskId}, clusterName=${urlClusterName}`);
    ...
    setTimeout(() => {
      if (isCreating) {
        ...
        checkIfClusterExists().then(exists => {
          if (exists) {
            console.log('Auto-completion found that cluster exists, marking as success');
            setProgress(100);
            setSuccess(true);
            setIsCreating(false);
            ...
            toast({
              ...
              description: "Cluster creation may have completed. Redirecting to dashboard to check status...",
              duration: 5000
            });
          }
  ...
  const pollTaskStatus = useCallback((taskId: string, targetClusterName?: string) => {
    const effectiveClusterName = targetClusterName || clusterName;
    console.log(`🔄 Starting task polling for task ID: ${taskId} for cluster: ${effectiveClusterName}`);
    ...
    clusterApi.pollTaskStatus(
      ...
      (status) => {
        ...
        if (status.success || status.status === 'completed') {
          ...
          checkIfClusterExists(effectiveClusterName).then(exists => {
            if (exists) {
              ...
              setStatusMessage('Cluster creation completed but verification failed');
              setIsCreating(false);
              setError('Cluster creation completed but cluster verification failed');

              toast({
                title: "Warning",
                description: "Cluster creation completed but verification failed. Check dashboard for status.",
                variant: "destructive",
                duration: 5000,
              });
            }
          ...
          console.log('🔍 DEBUG: Checking for backend timeout patterns in message:', status.message);
          ...
          console.log('🔍 DEBUG: Backend timeout detection:', {
            ...
            isBackendTimeout,
            matchedPatterns: timeoutPatterns.filter(pattern => status.message?.includes(pattern))
          });

          if (isBackendTimeout) {
            console.log('⏰ Backend timeout detected, checking if cluster exists despite failure...');
            setCreationPhase('verifying');
            setStatusMessage('Backend timeout detected, verifying cluster status...');

            // Check if cluster exists despite backend timeout
            checkIfClusterExists().then(exists => {
              if (exists) {
                console.log('✅ Cluster exists despite backend timeout, marking as success');
                setCreationPhase('completed');
                setStatusMessage('Cluster created successfully!');
                setProgress(100);
                setSuccess(true);
                ...
                setCreationPhase('failed');
                setStatusMessage(`Failed: ${status.message || 'Cluster creation failed'}`);
                setIsCreating(false);
              ...
              console.error('Error checking cluster existence after backend timeout:', checkError);
      ...
      (error) => {
        ...
        const isPollingTimeout = pollingTimeoutPatterns.some(pattern =>
          error.message?.includes(pattern)
        ) || error.name === 'TaskPollingTimeout' || (error as any).isTaskTimeout;
        ...
        if (isPollingTimeout) {
          console.log('⏰ Task polling timed out, checking if cluster exists...');
          setCreationPhase('verifying');
          setStatusMessage('Task polling timed out, verifying cluster status...');

          // Check if cluster exists despite timeout
          checkIfClusterExists().then(exists => {
            if (exists) {
              console.log('✅ Cluster exists despite timeout, marking as success');
              setCreationPhase('completed');
              setStatusMessage('Cluster created successfully!');
              setProgress(100);
              setSuccess(true);
              ...
              setCreationPhase('failed');
              setStatusMessage('Cluster creation timed out and cluster was not found');
              setIsCreating(false);
              setError('Cluster creation timed out');

              toast({
                title: "Timeout",
                description: 'Cluster creation timed out. Please check the dashboard or try again.',
                variant: "destructive",
                duration: 5000,
              });
            }
            ...
            setCreationPhase('failed');
            setStatusMessage('Unable to verify cluster status after timeout');
            setIsCreating(false);
            setError('Unable to verify cluster status');

            toast({
              title: "Error",
              description: 'Unable to verify cluster status. Please check the dashboard.',
              variant: "destructive",
              duration: 5000,
            });
          });
          ...
          setCreationPhase('failed');
          setStatusMessage(`Polling failed: ${error.message || 'Failed to track cluster creation progress'}`);
          setIsCreating(false);
          setError(error.message || 'Failed to track cluster creation progress');

          toast({
            title: "Error",
            description: error.message || 'Failed to track cluster creation progress',
            variant: "destructive",
            duration: 5000,
          });
        }
  ...
  const handleCreateCluster = async () => {
    ...
    if (showAdvancedOptions) {
      if (!workerCpu || parseInt(workerCpu) < 1 || parseInt(workerCpu) > 8) {
        setError('Please enter a valid number of CPU cores for worker nodes (1-8)');
        return;
      }

      if (!workerMemory || parseInt(workerMemory) < 1 || parseInt(workerMemory) > 16) {
        setError('Please enter a valid amount of memory for worker nodes (1-16 GB)');
        return;
      }

      if (!controlPlaneCpu || parseInt(controlPlaneCpu) < 1 || parseInt(controlPlaneCpu) > 8) {
        setError('Please enter a valid number of CPU cores for control plane (1-8)');
        return;
      }

      if (!controlPlaneMemory || parseInt(controlPlaneMemory) < 1 || parseInt(controlPlaneMemory) > 16) {
        setError('Please enter a valid amount of memory for control plane (1-16 GB)');
        return;
      }
    }
    ...
    try {
      ...
      if (exists) {
        console.log(`Cluster '${clusterName}' already exists, marking as success`);
        setCreationPhase('completed');
      ...
      if (result && result.task_id) {
        ...
        toast({
          title: "Success",
          description: result.message || `Cluster '${clusterName}' created successfully!`,
          duration: 5000,
        });
        ...
        const errorMessage = result?.message || 'Cluster creation failed - no task ID received';
        console.error('Cluster creation failed:', errorMessage, 'Full result:', result);
        setCreationPhase('failed');
  ...
  const checkIfClusterExists = async (targetClusterName?: string): Promise<boolean> => {
    const effectiveClusterName = targetClusterName || clusterName;
    ...
    console.log(`🔍 Checking if cluster '${effectiveClusterName}' exists using enhanced cluster checker...`);
    ...
    try {
      ...
      console.log(`🔍 DEBUG: Cluster existence check completed in ${duration}ms: ${result ? 'EXISTS' : 'NOT FOUND'}`);
      ...
      try {
        ...
        if (response.ok) {
          ...
          const exists = clusters.some((cluster: any) => cluster.name === effectiveClusterName);
          console.log(`Simple fallback check result: ${exists ? 'EXISTS' : 'NOT FOUND'}`);
          return exists;
        }
      } catch (fallbackError) {
        ...
        {!isCreating ? (
          <div className="space-y-6">
            <div className="grid gap-4">
              <div className="flex flex-col space-y-1.5">
                <label htmlFor="name" className="font-medium">Cluster Name</label>
                <input
                  id="name"
                  value={clusterName}
              ...
              <div className="flex flex-col space-y-1.5">
                <label htmlFor="workers" className="font-medium">Worker Nodes</label>
                <input
                  id="workers"
                  type="number"
                  min="1"
            ...
            {showAdvancedOptions && (
              <div className="mt-6 space-y-6 border-t pt-6">
                ...
                <p className="text-sm text-muted-foreground mb-4">Fine-tune resource allocation and other advanced settings</p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col space-y-1.5">
                    <label htmlFor="workerCpu" className="font-medium">Worker Node CPU Cores</label>
                    <input
                      id="workerCpu"
                      type="number"
                      min="1"
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground"
                    />
                    <p className="text-xs text-muted-foreground">Number of CPU cores per worker node</p>
                  </div>

                  <div className="flex flex-col space-y-1.5">
                    <label htmlFor="workerMemory" className="font-medium">Worker Node Memory (GB)</label>
                    <input
                      id="workerMemory"
                      type="number"
                      min="1"
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground"
                    />
                    <p className="text-xs text-muted-foreground">Memory allocation per worker node in GB</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col space-y-1.5">
                    <label htmlFor="controlPlaneCpu" className="font-medium">Control Plane CPU Cores</label>
                    <input
                      id="controlPlaneCpu"
                      type="number"
                      min="1"
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground"
                    />
                    <p className="text-xs text-muted-foreground">Number of CPU cores for control plane node</p>
                  </div>

                  <div className="flex flex-col space-y-1.5">
                    <label htmlFor="controlPlaneMemory" className="font-medium">Control Plane Memory (GB)</label>
                    <input
                      ...
                      className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground"
                    />
                    <p className="text-xs text-muted-foreground">Memory allocation for control plane node in GB</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col space-y-1.5">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                    />
                    <label htmlFor="applyResourceLimits" className="font-medium">
                      Apply resource limits to containers
                    </label>
                  </div>
                  <p className="text-xs text-muted-foreground ml-6">Enforce CPU and memory limits on containers</p>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-end">
              <Button onClick={handleCreateCluster} className="bg-primary text-white px-6">
                Create Cluster
              </Button>
            </div>
          </div>
          ...
          <div className="bg-card border rounded-lg p-6 shadow-sm">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-medium">{success ? 'Cluster Created!' : 'Creating Cluster...'}</h3>
                <p className="text-muted-foreground">Please wait while we set up your cluster</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center justify-between text-sm">
                <p className="text-muted-foreground">
                  {statusMessage || (
                    creationPhase === 'starting' ? 'Preparing cluster configuration...' :
                    creationPhase === 'creating' ? 'Initiating cluster creation...' :
                    creationPhase === 'polling' ? 'Monitoring cluster creation progress...' :
                    creationPhase === 'verifying' ? 'Verifying cluster status...' :
                    creationPhase === 'completed' ? 'Cluster created successfully!' :
                    creationPhase === 'failed' ? 'Cluster creation failed' :
                    progress < 30 ? 'Initializing cluster...' :
                    progress >= 30 && progress < 60 ? 'Creating control plane...' :
                    progress >= 60 && progress < 90 ? 'Creating worker nodes...' :
                    'Finalizing setup...'
                  )}
                </p>
            ...
            {showSkipButton && (
              <div className="mt-6 flex justify-end">
                <Button
                onClick={async () => {
                  console.log('User skipped waiting, checking if cluster exists');
                  ...
                  try {
                    ...
                    toast({
                      ...
                      description: "Verifying if cluster creation has completed",
                      duration: 3000,
                    });

                    // Check if the cluster exists with timeout
                    console.log(`Checking if cluster '${clusterName}' exists for skip navigation...`);
                    const exists = await Promise.race([
                      checkIfClusterExists(),
                      new Promise<boolean>((_, reject) =>
                        setTimeout(() => reject(new Error('Cluster check timeout')), 15000)
                      )
                    ]);

                    if (exists) {
                      console.log('✅ Cluster exists, marking as success and redirecting');
                      setIsCreating(false);
                      setProgress(100);
                      setSuccess(true);
                      ...
                      console.log('❌ Cluster does not exist yet, going to dashboard anyway');
                      setIsCreating(false);

                      toast({
                        title: "Proceeding to Dashboard",
                        description: "Cluster creation may still be in progress. Check the dashboard for updates.",
                        duration: 4000,
                      });
                    }
                  } catch (error) {
                    console.error('Error checking cluster status for skip navigation:', error);
                    setIsCreating(false);

                    toast({
                      title: "Proceeding to Dashboard",
                      description: "Unable to verify cluster status. Redirecting to dashboard...",
                      duration: 4000,
                    });
                  } finally {
                    // Reset loading state
            ...
            {success && (
              <div className="mt-6 bg-success/10 p-4 rounded-md flex items-start gap-3 text-success border border-success/20">
                ...
                <div>
                  ...
                  <p className="text-sm">Your cluster '{clusterName}' has been created successfully.</p>
                </div>
              </div>
            )}
          </div>
