...
export function RootLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fixStylesheets = () => {
      ...
      document.querySelectorAll('style[media="x"]').forEach(styleTag => {
    ...
    linkElements.forEach(link => {
      ...
      if (
        ...
        (href.includes('_next/static/css') ||
          href.includes('app-build-manifest'))
      ) {
        // Force reload the stylesheet with cache-busting
        const newLink = document.createElement('link');
        newLink.rel = 'stylesheet';
        newLink.href =
          href + (href.includes('?') ? '&' : '?') + 'reload=' + Date.now();
    ...
    <Providers>
      <div className='min-h-screen flex flex-col bg-background'>
        ...
        <div className='flex-grow flex relative'>
          ...
          {isLargeScreen && (
            <aside
              className='hidden md:block fixed top-[57px] bottom-0 left-0 w-64 z-40
          ...
          <main
            className={`flex-grow p-3 sm:p-5 md:p-6 lg:p-8 w-full overflow-x-hidden transition-all duration-300 ${isLargeScreen ? 'md:ml-64' : 'ml-0'}`}
          >
            <motion.div
