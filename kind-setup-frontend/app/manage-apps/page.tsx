...
const getAccessUrlIcon = (type: string, appName: string) => {
  ...
  if (
    appName.toLowerCase().includes('database') ||
...
export default function ManageApps() {
  ...
  const isSystemComponent = (app: Application): boolean => {
    ...
    return (
      systemNamespaces.includes(app.namespace || '') ||
  ...
  const loadClusters = useCallback(async () => {
    try {
      ...
      if (clusterStatus.clusters && Array.isArray(clusterStatus.clusters)) {
        const clusterNames = clusterStatus.clusters.map(
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      if (clustersResult.clusters && Array.isArray(clustersResult.clusters)) {
        const clusterNames = clustersResult.clusters.map(
          cluster => cluster.name
        );
        setAvailableClusters(clusterNames);
      }
  ...
  const handleAppAction = async (app: Application, action: string) => {
    ...
    try {
      await clusterApi.manageApplication(
        app.id,
        action,
        app.cluster || 'default',
...
Are you absolutely sure you want to proceed?`;
  ...
  const handleScaleApp = async () => {
    ...
    try {
      await clusterApi.scaleApplication(
        selectedApp.id,
        scaleReplicas,
        selectedApp.cluster || 'default',
        selectedApp.namespace || 'default'
      );
      toast({
        title: 'Success',
  ...
  const fetchAppDetails = async (app: Application) => {
    try {
      const response = await clusterApi.getApplicationDetails(
        app.id,
        app.cluster || 'test-cluster',
        app.namespace || 'dev'
      );
  ...
  const fetchAppLogs = async (app: Application) => {
    try {
      const response = await clusterApi.getApplicationLogs(
        app.id,
        app.cluster || 'test-cluster',
        app.namespace || 'dev',
        200
...
Please check the backend server status and try again.`);
  ...
  const fetchAppMetrics = async (app: Application) => {
    try {
      const response = await clusterApi.getApplicationMetrics(
        app.id,
        app.cluster || 'test-cluster',
        app.namespace || 'dev'
      );
            ...
            <div className='flex items-center space-x-2 pt-2 border-t'>
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setHideSystemComponents(checked === true)
                }
              />
              <Label
            ...
            <CardContent className='pt-6'>
              <div className='flex flex-col items-center justify-center text-center p-8'>
                ...
                {apps.length === 0 && (
                  <Button
                    className='mt-4'
                    onClick={() => (window.location.href = '/deploy-app')}
                  >
                ...
                <div className='flex flex-wrap gap-2 mb-4'>
                  ...
                  {app.access_urls && app.access_urls.length > 0 && (
                    <Button
                      ...
                      size='sm'
                      onClick={() =>
                        window.open(app.access_urls![0].url, '_blank')
                      }
                  ...
                  {(() => {
                    const appKey = `${app.id}-${app.cluster}-${app.namespace}`;
                    const isPortForwardActive =
                      portForwardStatus[appKey]?.status === 'active';
                    ...
                    return (
                      <Button
                        ...
                        onClick={() =>
                          isPortForwardActive
                            ? handleStopPortForward(app)
                            : handleStartPortForward(app)
                        }
                        ...
                        title={
                          isPortForwardActive
                            ? `Stop port forwarding (${portForwardStatus[appKey]?.access_url})`
                            : 'Start port forwarding to access application UI'
                        }
                      >
                        {isLoading ? (
                  ...
                  {(() => {
                    ...
                    if (
                      portForwardInfo?.status === 'active' &&
                      portForwardInfo.access_url
                    ) {
                      return (
                        <Button
                          variant='default'
                          size='sm'
                          onClick={() =>
                            window.open(portForwardInfo.access_url, '_blank')
                          }
                ...
                <div className='flex flex-wrap gap-2 mb-4'>
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      actionLoading === `${app.id}-delete-all`
                    }
                  ...
                  <div className='border-t pt-4'>
                    ...
                    <div className='space-y-2'>
                      {app.access_urls.map((url, index) => {
                        const IconComponent = getAccessUrlIcon(
                          url.type,
                          app.name
                        );
                        ...
                        return (
                          <div
                            key={index}
                            className='flex justify-between items-center p-3 bg-secondary/20 rounded-md hover:bg-secondary/30 transition-colors border border-secondary/40'
                          >
                            <div className='flex items-center gap-3 flex-1'>
                              ...
                              <div className='flex-1 min-w-0'>
                                ...
                                <p
                                  className='text-sm font-mono text-blue-600 hover:text-blue-800 cursor-pointer truncate'
                  ...
                  <div className='space-y-2'>
                    <div className='grid grid-cols-2 gap-4'>
                      <div>
                        ...
                        <p className='text-sm font-medium'>
                          {appDetails.deployment.replicas || 0}
                        </p>
                      </div>
                      <div>
                        <Label>Ready Replicas</Label>
                        <p className='text-sm font-medium'>
                          {appDetails.deployment.ready_replicas || 0}
                        </p>
                      </div>
                      <div>
                        <Label>Available Replicas</Label>
                        <p className='text-sm font-medium'>
                          {appDetails.deployment.available_replicas || 0}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <TabsContent value='access' className='space-y-4'>
                {appDetails && appDetails.access_info ? (
                  <div className='space-y-6'>
                    ...
                    {appDetails.access_info.connection_info &&
                      Object.keys(appDetails.access_info.connection_info)
                        .length > 0 && (
                        <div>
                                ...
                                {appDetails.access_info.connection_info
                                  ...
                                  <div className='space-y-2'>
                                    ...
                                    <ul className='text-sm text-muted-foreground space-y-1'>
                                      {appDetails.access_info.connection_info.notes.map(
                                        (note, index) => (
                                          <li
                                            key={index}
                    ...
                    {appDetails.access_info.port_forward_commands &&
                      appDetails.access_info.port_forward_commands.length >
                        0 && (
                        <div>
                          <h4 className='text-sm font-medium mb-3'>
                            Port Forward Commands
                          ...
                          <div className='space-y-2'>
                            {appDetails.access_info.port_forward_commands.map(
                              (cmd, index) => (
                                <Card key={index}>
                                  <CardContent className='pt-4'>
                                    <div className='space-y-2'>
                                      <div className='flex justify-between items-center'>
                                        <div>
                                          ...
                                          <p className='text-xs text-muted-foreground'>
                                            {cmd.port_name} - {cmd.service_port}{' '}
                                            → {cmd.local_port}
                                          </p>
                                        </div>
      ...
      <Dialog open={logsDialogOpen} onOpenChange={setLogsDialogOpen}>
        <DialogContent className='max-w-4xl max-h-[80vh]'>
          ...
          <DialogFooter>
            <Button
              variant='outline'
              onClick={() => selectedApp && fetchAppLogs(selectedApp)}
            >
              Refresh Logs
            </Button>
          </DialogFooter>
      ...
      <Dialog open={metricsDialogOpen} onOpenChange={setMetricsDialogOpen}>
        <DialogContent className='max-w-3xl'>
            ...
            <div className='space-y-6'>
                        ...
                        <div className='space-y-1'>
                          <div className='flex justify-between'>
                            ...
                            <span className='text-sm font-medium'>
                              {appMetrics.resources?.requests?.cpu || 'Not set'}
                            </span>
                          </div>
                          <div className='flex justify-between'>
                            <span className='text-sm text-muted-foreground'>
                            ...
                            <span className='text-sm font-medium'>
                              {appMetrics.resources?.requests?.memory ||
                                'Not set'}
                            </span>
                          </div>
                        </div>
                      ...
                      <CardContent>
                        <div className='space-y-1'>
                          <div className='flex justify-between'>
                            ...
                            <span className='text-sm font-medium'>
                              {appMetrics.resources?.limits?.cpu || 'Not set'}
                            </span>
                          </div>
                          <div className='flex justify-between'>
                            <span className='text-sm text-muted-foreground'>
                            ...
                            <span className='text-sm font-medium'>
                              {appMetrics.resources?.limits?.memory ||
                                'Not set'}
                            </span>
                          </div>
                        </div>
              ...
              <div>
                ...
                {appMetrics.metrics && appMetrics.metrics.length > 0 ? (
                  ...
                  <p className='text-sm text-muted-foreground'>
                    No metrics available. Make sure metrics-server is installed
                    in your cluster.
                  </p>
                )}
              </div>
          ...
          <DialogFooter>
            <Button
              variant='outline'
              onClick={() => selectedApp && fetchAppMetrics(selectedApp)}
            >
              Refresh Metrics
            </Button>
          </DialogFooter>
      ...
      <Dialog open={scaleDialogOpen} onOpenChange={setScaleDialogOpen}>
        <DialogContent className='max-w-md'>
          ...
          <div className='space-y-4'>
            <div>
              ...
              <p className='text-xs text-muted-foreground mt-1'>
                Set to 0 to stop the application, or increase for high
                availability
              </p>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            >
              {actionLoading === `${selectedApp?.id}-scale`
                ? 'Scaling...'
                : 'Scale Application'}
            </Button>
          </DialogFooter>
