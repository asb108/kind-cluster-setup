...
from typing import Dict, List, Optional, Any, Union, Tuple, Iterator, ContextManager, Callable, TypeVar
...
from kind_cluster_setup.core.command import CommandExecutor, SubprocessCommandExecutor, CommandResult, MockCommandExecutor
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            while attempt <= max_attempts:
                try:
                    ...
                    logger.warning(f"Attempt {attempt} failed: {str(e)}. Retrying in {current_delay:.2f}s...")
...
class KindCluster:
    ...
    def __init__(self,
                ...
                executor: Optional[CommandExecutor] = None) -> None:
        """
        ...
        if 'control_plane_config' in cluster_config:
            logger.info(f"Control plane config detected: {cluster_config['control_plane_config']}")

        self.cluster_config = cluster_config
        self.env_config = env_config
        self.cluster_name = cluster_config.get('name', f"kind-cluster-{env_config.get('namespace', 'dev')}")
        ...
        self.https_port = custom_ports.get('https', cluster_config.get('https_port', 443))
        self.nodeport_start = custom_ports.get('nodeport', cluster_config.get('nodeport_start', 30000))
    ...
    def _find_available_port_range(self, start_port: int, count: int = 1, step: int = 1) -> List[int]:
    ...
    @retry(max_attempts=3, delay=2.0, exceptions=(DockerNotRunningError, KindNotInstalledError, ClusterOperationError))
    def _check_port_availability(self) -> None:
        ...
        def is_port_free(port):
            ...
            try:
                ...
                return result.returncode != 0  # Return code 1 means no process using this port (good)
            except Exception:
        ...
        if not is_port_free(self.http_port):
            raise ClusterOperationError(f"Unable to find free alternative for HTTP port {self.http_port}")
        if not is_port_free(self.https_port):
            raise ClusterOperationError(f"Unable to find free alternative for HTTPS port {self.https_port}")
        if not is_port_free(self.nodeport_start):
            raise ClusterOperationError(f"Unable to find free alternative for NodePort {self.nodeport_start}")

        logger.info(f"Using ports - HTTP: {self.http_port}, HTTPS: {self.https_port}, NodePort: {self.nodeport_start}")
    ...
    def _create_kind_config(self) -> Dict[str, Any]:
        ...
        control_plane_node = {
            ...
            'extraPortMappings': [
                ...
                {'containerPort': 30080, 'hostPort': self.nodeport_start, 'protocol': 'TCP'}
            ]
        }
    ...
    def _parse_memory_to_bytes(self, memory_str: str) -> int:
        ...
        if 'G' in memory_str:
            return int(float(memory_str.replace('GB', '').replace('G', '')) * 1024 * 1024 * 1024)
        elif 'M' in memory_str:
            return int(float(memory_str.replace('MB', '').replace('M', '')) * 1024 * 1024)
    ...
    def _apply_resource_limits(self) -> None:
        ...
        try:
            ...
            logger.info(f"Applying resource limits to control plane: CPU={cp_cpus}, Memory={cp_memory} (bytes: {cp_memory_bytes})")
        ...
        try:
            ...
            if isinstance(self.executor, MockCommandExecutor):
                for i in range(worker_nodes):
                    ...
                    logger.info(f"Applying resource limits to worker {i+1}: CPU={worker_cpus}, Memory={worker_memory} (bytes: {worker_memory_bytes})")
                    self.docker_client.update_container(
    ...
    def create(self) -> bool:
        ...
        try:
            ...
            if self.cluster_name in existing_clusters:
                logger.info(f"Cluster '{self.cluster_name}' already exists, skipping creation.")
                self._created = True
                return True
            ...
            config_path = os.path.join(PROJECT_ROOT, f"kind_config_{self.cluster_name}.yaml")
            ...
            elif "failed to copy files" in error_message.lower() and "write" in error_message.lower():
                user_friendly_message = (
    ...
    def delete(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Checking if cluster '{self.cluster_name}' exists before deletion")
            existing_clusters = self.kind_client.get_clusters()

            if self.cluster_name not in existing_clusters:
                logger.warning(f"Cluster '{self.cluster_name}' does not exist, nothing to delete.")
                self._created = False
                return True
    ...
    def install_ingress(self, ingress_type: str = "nginx") -> bool:
        ...
        """
        logger.info(f"Installing {ingress_type} ingress controller in cluster '{self.cluster_name}'")
        ...
        try:
            ...
            if ingress_type.lower() == "nginx":
                ...
                result = self.kubectl_client.apply(
                    files=["https://raw.githubusercontent.com/kubernetes/ingress-nginx/main/deploy/static/provider/kind/deploy.yaml"],
                ...
                if not wait_result.success:
                    logger.error(f"Ingress controller pods did not become ready: {wait_result.stderr}")
                    return False
                ...
                raise ValueError(f"Unsupported ingress type: {ingress_type}. Supported types: nginx")
        except Exception as e:
    ...
    def wait_for_ready(self, timeout: int = 60) -> bool:
        ...
        """
        logger.info(f"Waiting up to {timeout} seconds for cluster '{self.cluster_name}' to be ready...")
        context_name = f"kind-{self.cluster_name}"
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                logger.info(f"Waiting for nodes to be ready. Current status: {statuses}")
                time.sleep(5)
                    ...
                    if len(parts) >= 4:
                        ...
                        for node_line in nodes_result.stdout.strip().split('\n')[1:]:  # Skip header
                            ...
                            if len(node_parts) >= 5 and node_parts[0] == node_name:
                                version = node_parts[4]  # Version is typically in the 5th column
                                break
    ...
    def port_forward(self, resource: str, port_mapping: str) -> subprocess.Popen:
        ...
        """
        context_name = f"kind-{self.cluster_name}"
        cmd = ["kubectl", "port-forward", resource, port_mapping, f"--context={context_name}"]
        ...
        if process.poll() is not None:
            ...
            raise ClusterOperationError(f"Failed to establish port forwarding: {stderr}")
