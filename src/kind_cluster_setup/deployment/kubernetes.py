...
class KubernetesDeploymentStrategy(DeploymentStrategy):
    ...
    def deploy(self, app: str, app_config: Union[Dict[str, Any], List[Dict[str, Any]]], env_config: Dict[str, Any],
        ...
        namespace = app_config.get('namespace') or env_config.get('namespace', f"{app}-{env_config.get('environment', 'dev')}")
        ...
        if template_dir is not None and os.path.exists(template_dir):
            ...
            logger.info(f"Deploying to namespace: {namespace} in cluster: {cluster_name}")

            # Process templates and apply to cluster
            with tempfile.TemporaryDirectory() as temp_dir:
                processed_files = self._process_template_files(template_dir, temp_dir, values)
                ...
                try:
                    ...
                    logger.info(f"Successfully applied {app} templates: {result.stdout}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    logger.info(f"Successfully applied {app} configuration: {result.stdout}")
                except Exception as e:
        ...
        if expose_service:
            ...
            try:
                ...
                if service_type == 'NodePort' and 'spec' in service_details and 'ports' in service_details['spec']:
                    ...
                    if node_port:
                        ...
                        if context.startswith('kind-'):
                            logger.info(f"Kind cluster detected. To access the service, run:")
                            logger.info(f"kubectl port-forward service/{app} {node_port}:{service_port} -n {namespace}")
            except Exception as e:
    ...
    def _set_kubectl_context(self, context_name: str) -> None:
        ...
        try:
            ...
            check_context = self.kubectl_client.execute(
                ["config", "get-context", context_name],
                check=False
            )
            ...
            if check_context.success:
                ...
                current_context = self.kubectl_client.execute(["config", "current-context"])
                current_context_name = current_context.stdout.strip()
                logger.warning(f"Context {context_name} not found, using current context: {current_context_name}")
        except Exception as e:
    ...
    def _create_namespace_if_not_exists(self, namespace: str, context: Optional[str] = None) -> None:
    ...
    def _process_template_files(self, template_dir: str, output_dir: str, values: Dict[str, Any]) -> List[str]:
        ...
        for filename in os.listdir(template_dir):
            if filename.endswith(('.yaml', '.yml')) and not filename.startswith('.'):
                ...
                try:
                    ...
                    for key, value in values.items():
                        ...
                        for placeholder in placeholders:
                            ...
                            if isinstance(value, (dict, list)):
                                ...
                                content = content.replace(placeholder, yaml_value.rstrip())
                            else:
                    ...
                    conditional_pattern = r'\{\{-\s*if\s*\.(\w+)\s*\}\}(.*?)\{\{-\s*end\s*\}\}'
                    def conditional_replace(match):
                        ...
                        return ""
                    content = re.sub(conditional_pattern, conditional_replace, content, flags=re.DOTALL)

                    # Validate the resulting YAML
                    try:
                        yaml.safe_load(content)
                    except yaml.YAMLError as e:
                        logger.warning(f"Generated YAML for {filename} may be invalid: {e}")
                    ...
                    logger.error(f"Failed to process template file {filename}: {str(e)}")

        return processed_files

    def _wait_for_deployments(self, app: str, namespace: str, context: Optional[str] = None, timeout: int = 300) -> None:
        """Wait for deployments to be ready.
        ...
        try:
            ...
            logger.warning(f"Timed out waiting for {app} deployments to be ready: {str(e)}")

    def check_status(self, app: str, env_config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for pod in pods_data.get("items", []):
                ...
                for container in pod.get("spec", {}).get("containers", []):
                    ...
                    container_status = next((c for c in status.get("containerStatuses", []) if c.get("name") == container_name), {})
            ...
            for svc in services_data.get("items", []):
                ...
                external_ips = svc.get("status", {}).get("loadBalancer", {}).get("ingress", [])
            ...
            try:
                ...
                logger.info(f"Detected {'Kind' if is_kind_cluster else 'non-Kind'} cluster")
            except Exception as e:
                logger.warning(f"Failed to determine cluster type, assuming Kind cluster: {str(e)}")
                            ...
                            if svc.get("type") == "NodePort":
                                for port in svc.get("ports", []):
                                    if "node_port" in port:
                                        logger.info(f"Using localhost NodePort instead of ingress host for Kind cluster")
                                        access_urls.append({
                                            "type": "nodeport",
                ...
                # For NodePort services
                    ...
                    for port in svc.get("ports", []):
                        if "node_port" in port:
                            ...
                            if is_kind_cluster:
                                node_ip = "localhost"
                                logger.info(f"Using localhost for Kind cluster NodePort service")
                            else:
                                ...
                                try:
                                    ...
                                    logger.error(f"Failed to determine node IP: {str(e)}")
                            ...
                            logger.info(f"Added NodePort access URL: http://{node_ip}:{port.get('node_port')}")
            ...
            logger.error(f"Failed to check status of Kubernetes deployment for {app}: {str(e)}")
            return {
                "app": app,
                "namespace": namespace,
                "status": "Error",
