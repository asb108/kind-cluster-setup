...
from kind_cluster_setup.utils.constants import DEFAULT_CLUSTER_CONFIG, DEFAULT_ENV_CONFIG
...
class CreateCommand(Command):
    ...
    def execute(self, args: argparse.Namespace) -> dict:
        ...
        try:
            ...
            logger.info(f"Creating Kind cluster with config: {cluster_config} and env: {env_config}")
            ...
            if apply_resource_limits and (worker_config or control_plane_config):
                ...
                if resource_result.get('status') != 'success':
                    logger.warning(f"Failed to apply resource limits: {resource_result.get('error')}")

            # Create a cluster entity and save it to the repository
            cluster = self._create_cluster_entity(cluster_name, cluster_config, env_config)
            ...
            if "docker has run out of disk space" in error_message.lower() or "no space left on device" in error_message.lower():
                user_friendly_error = (
            ...
            elif "docker storage is full" in error_message.lower() or "failed to copy files" in error_message.lower():
                user_friendly_error = (
    ...
    def _create_cluster_entity(self, cluster_name: str, cluster_config: dict, 
                             env_config: dict) -> Optional[Cluster]:
        """
        Create a cluster entity and save it to the repository.
    ...
    def _update_task_status(self, task: Optional[Task], status: str, 
                          result: Dict[str, Any]) -> Optional[Task]:
        """
    ...
    def _apply_resource_limits(self, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Applying resource limits to cluster {getattr(args, 'name', 'unknown')}")
            ...
            if hasattr(args, 'control_plane_config') and getattr(args, 'control_plane_config'):
                ...
                try:
                    ...
                    if hasattr(args.control_plane_config, 'cpu'):
                        update_params['cpu_quota'] = int(float(getattr(args.control_plane_config, 'cpu')) * 100000)
                    
                    # Handle memory limits
                    if hasattr(args.control_plane_config, 'memory'):
                        update_params['mem_limit'] = str(parse_memory(getattr(args.control_plane_config, 'memory'))) + 'b'
                    
                    # Handle memory swap (must be set with memory)
                    if hasattr(args.control_plane_config, 'memory_swap'):
                        update_params['memswap_limit'] = str(parse_memory(getattr(args.control_plane_config, 'memory_swap'))) + 'b'
                    
                    if update_params:
                        container.update(**update_params)
                        logger.info(f"Applied resource limits to control plane node {control_plane_name}: {update_params}")
                    else:
                        logger.warning("No valid resource limits provided for control plane")
                        
                except Exception as e:
                    error_msg = f"Failed to apply resource limits to control plane: {str(e)}"
            ...
            if hasattr(args, 'worker_config') and getattr(args, 'worker_config'):
                ...
                for i in range(1, worker_nodes + 1):
                    worker_name = f"{getattr(args, 'name')}-worker" if i == 1 else f"{getattr(args, 'name')}-worker{i}"
                    try:
                        ...
                        if hasattr(args.worker_config, 'cpu'):
                            update_params['cpu_quota'] = int(float(getattr(args.worker_config, 'cpu')) * 100000)
                        
                        # Handle memory limits
                        if hasattr(args.worker_config, 'memory'):
                            update_params['mem_limit'] = str(parse_memory(getattr(args.worker_config, 'memory'))) + 'b'
                        
                        # Handle memory swap (must be set with memory)
                        if hasattr(args.worker_config, 'memory_swap'):
                            update_params['memswap_limit'] = str(parse_memory(getattr(args.worker_config, 'memory_swap'))) + 'b'
                        
                        if update_params:
                            container.update(**update_params)
                            logger.info(f"Applied resource limits to worker node {worker_name}: {update_params}")
                        else:
                            logger.warning(f"No valid resource limits provided for worker node {worker_name}")
    ...
    def _create_cluster(self, cluster_config: Dict[str, Any], env_config: Dict[str, Any], 
                       worker_config: Dict[str, Any], control_plane_config: Dict[str, Any],
                       cluster_name: str, worker_nodes: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Apply resource limits if needed
            if cluster_config.get('apply_resource_limits', False) and (worker_config or control_plane_config):
                logger.info("Applying resource limits to cluster nodes...")
    ...
    def _create_cluster_entity(self, cluster_name: str, cluster_config: dict, env_config: dict) -> Optional[Cluster]:
        """
        Create a cluster entity and save it to the repository.
        ...
        if self.cluster_repository is None:
            logger.warning("Cluster repository not available, skipping cluster entity creation")
            return None
        ...
        if existing_cluster:
            ...
            existing_cluster.environment = env_config.get('environment', DEFAULT_ENV_CONFIG['environment'])
