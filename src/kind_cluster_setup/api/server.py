    ...
    try:
        ...
        return JSONResponse(
            ...
            content=create_api_response(
                API_STATUS["success"] if health_info['status'] == 'healthy' else API_STATUS["warning"],
...
class CustomPorts(BaseModel):
    ...
    nodeport_start: Optional[int] = None  # Default NodePort range start (30000 or alternative)
...
async def create_cluster_task(
    ...
    try:
        ...
        result = {}
        try:
            logger.info(f"Creating command with parameters: name={cluster_name}, worker_nodes={worker_nodes}, apply_resource_limits={apply_resource_limits}")
        ...
        if apply_resource_limits and (worker_config or control_plane_config):
            ...
            try:
                ...
                logger.info(f"Resource limits applied successfully: {resource_limits_result}")
        ...
        has_error = result.get("error") is not None or result.get("cluster_creation") == "failed"
        
        # Check if Docker containers exist for this cluster (as a final validation)
        try:
            import subprocess
            containers = subprocess.run(
                ["docker", "ps", "-a", "--filter", f"name={cluster_name}", "--format", "{{{{.Names}}}}"],
            ...
            if containers.stdout.strip() and not has_error:
                logger.info(f"Found containers for cluster {cluster_name}: {containers.stdout.strip()}")
                success = True
            ...
            message = "Cluster created successfully" if success else f"Cluster creation failed: {result.get('error', 'Unknown error')}"
        ...
        TASK_STATUSES[task_id] = task_status
        task_store.save_task(task_id, task_status)
        logger.info(f"Task {task_id} completed with status: {status}, success: {success}")
        ...
        try:
            ...
            if containers.stdout.strip():
                logger.warning(f"Found orphaned containers: {containers.stdout}. Consider cleaning them up.")
        except Exception:
            pass
    ...
    try:
        ...
        task_status = {
            ...
            "details": {
                ...
                "custom_ports": cluster.custom_ports.dict() if cluster.custom_ports else None
            },
            "progress": 0,
            "result": None
        }
...
def get_task_message(task_type: str, status: str, format_args: Dict[str, Any] = None) -> str:
...
def deploy_application_task(task_id: str, deployment_config: Dict[str, Any]):
    ...
    try:
        ...
        task_status = {
            "status": "running",
            "message": get_task_message("app_deployment", "preparing", {
        ...
        task_status = {
            "status": "running",
            "message": get_task_message("app_deployment", "deploying", {
                "app_name": deployment_config['app_name'],
        ...
        args = Namespace(
            ...
            deployments=[deployment_config.get('deployment_method', 'kubernetes')],  # DeployCommand expects this attribute
            cluster_name=deployment_config['cluster_name'],
        ...
        if not template:
            raise ValueError(f"Template not found for application: {deployment_config['app_name']}")
        ...
        from pathlib import Path
        project_root = Path(__file__).parent.parent.parent.parent
        base_template_dir = project_root / "templates" / "apps" / deployment_config['app_name']
        ...
        if deployment_method in ["kubernetes", "kubectl"]:
            ...
            template_dir = kubernetes_dir if kubernetes_dir.exists() else base_template_dir
        else:
        ...
        task_status = {
            "status": "completed",
            "message": get_task_message("app_deployment", "completed", {
                "app_name": deployment_config['app_name'],
                "cluster_name": deployment_config['cluster_name']
            }),
            "completed": True,
        ...
        task_status = {
            "status": "failed",
            "message": get_task_message("app_deployment", "failed", {
...
async def deploy_application(deployment: ApplicationDeploy, background_tasks: BackgroundTasks):
    """
    ...
    """
    logger.info(f"Deploying {deployment.app_name} to cluster {deployment.cluster_name} using {deployment.deployment_method}")
    ...
    try:
        ...
        thread = Thread(
            target=deploy_application_task,
            args=(task_id, deployment.dict())
        )
        thread.daemon = True
        thread.start()
        ...
        return create_api_response(
            API_STATUS["accepted"],
            get_task_message("app_deployment", "started", {
...
def apply_resource_limits(cluster_name: str, worker_config: Dict[str, Any] = None,
                      control_plane_config: Dict[str, Any] = None, worker_nodes: int = 1) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        cp_memory_bytes = int(control_plane_config['memory'].replace('GB', '')) * 1024 * 1024 * 1024
        ...
        cp_cmd = [
            "docker", "update",
            "--memory", str(cp_memory_bytes),
            "--memory-swap", str(cp_memory_swap),
            "--cpus", str(cp_cpus),
            cp_container
        ]
        ...
        # Set worker limits
        worker_memory_bytes = int(worker_config['memory'].replace('GB', '')) * 1024 * 1024 * 1024
        ...
        for i in range(worker_nodes):
            ...
            check_cmd = ["docker", "ps", "-a", "--filter", f"name={worker_container}", "--format", "{{{{.Names}}}}"]
            ...
            if check_result.stdout.strip():
                worker_cmd = [
                    ...
                    "--memory-swap", str(worker_memory_swap),
                    "--cpus", str(worker_cpus),
                    worker_container
                ]

                logger.info(f"Running command to set worker limits: {' '.join(worker_cmd)}")
                worker_result = subprocess.run(worker_cmd, capture_output=True, text=True)

                if worker_result.returncode == 0:
                    logger.info(f"Successfully applied worker limits to {worker_container}")
                    workers_updated += 1
                else:
                    logger.error(f"Failed to apply worker limits to {worker_container}: {worker_result.stderr}")
    ...
    try:
        ...
        for line in result.stdout.strip().split('\n'):
            if line:  # Skip empty lines
                clusters.append({
                    "name": line,
                    "status": "Running"  # Default status for now
                })
...
async def get_cluster_status(environments: Optional[str] = None):
    ...
    try:
        ...
        return create_api_response("success", "Cluster status retrieved successfully", data=response_data)
    except Exception as e:
...
async def task_debug(max_keys: int = 20):
    ...
    debug_data = {
        ...
        "sample_structure": [
            {"task_id": task_id, "type": TASK_STATUSES[task_id].get("type"), "status": TASK_STATUSES[task_id].get("status")}
            for task_id in list(TASK_STATUSES.keys())[:5]
        ] if TASK_STATUSES else []
    }

    return create_api_response("success", "Task debug information retrieved", data=debug_data)
...
async def start_port_forward(name: str, request: PortForwardRequest):
    ...
    """
    try:
        logger.info(f"Starting port forwarding for {request.resource} in cluster {name}")
...
async def get_app_deployment_status(app_id: str, cluster_name: str, namespace: str):
    ...
    try:
        ...
        if task and task.get('details', {}).get('app_name'):
            ...
            services_cmd = f"kubectl get services -n {namespace} --context {context} -o json"
            result = subprocess.run(services_cmd, shell=True, capture_output=True, text=True)

            if result.returncode == 0:
                import json
                services_data = json.loads(result.stdout)
                service_names = [s['metadata']['name'] for s in services_data.get('items', [])]
                ...
                if any('airflow' in name for name in service_names):
                    ...
                    result = subprocess.run(deployments_cmd, shell=True, capture_output=True, text=True)
                    if result.returncode == 0:
        ...
        if result.returncode == 0:
            ...
            for service in services_data.get('items', []):
                ...
                if app_name == 'airflow':
                    ...
                    should_include = (app_name.lower() in service_name.lower() or
                                    service_name.lower() in app_name.lower())
...
async def start_app_port_forward(app_id: str, request: AppPortForwardRequest):
    ...
    try:
        ...
        if not app_details or 'services' not in app_details:
            raise HTTPException(
                status_code=404,
                detail=f"No services found for application {app_id}"
            )
        ...
        for service in services:
            ...
            elif any(keyword in service_name.lower() for keyword in ['web', 'ui', 'frontend']):
                priority = 90
            ...
            if any(keyword in service_name.lower() for keyword in ['redis', 'postgres', 'mysql', 'db', 'cache']):
                priority = 1
        ...
        if service_priority:
            ...
            logger.info(f"Selected service '{target_service}' with priority {service_priority[0][0]} for app {app_id}")
            logger.info(f"Available services with priorities: {[(p, name) for p, name, _ in service_priority]}")
        ...
        if not local_port:
            ...
            while is_port_in_use(local_port):
                ...
                if local_port > 9000:  # Safety limit
                    raise HTTPException(
                        status_code=500,
                        detail="Could not find an available local port"
                    )
        ...
        cmd = [
            "kubectl", "port-forward", resource, port_mapping,
            f"--context={context_name}", f"--namespace={namespace}"
        ]
        ...
        if process.poll() is not None:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to start port forwarding: {error_msg}"
            )
...
async def stop_app_port_forward(app_id: str, cluster_name: str, namespace: str = "default"):
    """
...
async def get_app_port_forward_status(app_id: str, cluster_name: str, namespace: str = "default"):
    """
...
async def get_application_access_urls(cluster_name: str, namespace: str, app_name: str):
    ...
    try:
        ...
        port_forward_urls = await detect_port_forwarding(cluster_name, namespace, app_name)
        ...
        logger.info(f"Found {len(unique_urls)} access URLs for {app_name}: {[u['url'] for u in unique_urls]}")
        return unique_urls
                ...
                if line and (app_name in line or f"kind-{cluster_name}" in line):
                    ...
                    if 'service/' in line and ':' in line:
                        try:
                            ...
                            logger.debug(f"Error parsing port forward line: {line}, error: {e}")
...
async def get_real_kubernetes_applications():
    ...
    try:
        ...
        for cluster in clusters:
            ...
            user_namespaces = [ns for ns in namespaces if not ns.startswith(('kube-', 'local-path-storage')) or ns == 'default']
            ...
            for namespace in user_namespaces:
                ...
                for deployment in deployments_data.get('items', []):
                    ...
                    access_urls = await get_application_access_urls(cluster, namespace, app_name)
                    ...
                    app = {
                        ...
                        'status': 'Running' if status.get('readyReplicas', 0) > 0 else 'Stopped',
                        'pods': [],
                        'services': [],
                    ...
                    logger.info(f"Found real application: {app_name} in {namespace}@{cluster}")
    ...
    try:
        ...
        for cluster in clusters:
            try:
                ...
                system_namespaces = ['kube-system', 'local-path-storage', 'ingress-nginx', 'cert-manager']
                    ...
                    if result.returncode == 0:
                        ...
                        for deployment in deployments_data.get('items', []):
                            ...
                            if name:
                                ...
                                logger.debug(f"Added system component: {name} in {namespace}@{cluster}")

            except Exception as e:
                logger.error(f"Error getting system components from cluster {cluster}: {str(e)}")
                continue
...
async def list_applications(include_system_components: bool = False):
    ...
    """
    logger.info(f"Listing applications based on completed deployment tasks (include_system_components={include_system_components})")
    try:
        # Debug: Log keys to understand what we're working with
        task_keys = list(TASK_STATUSES.keys())
        logger.info(f"TASK_STATUSES has {len(task_keys)} tasks with keys: {task_keys[:10] if task_keys else 'EMPTY!'}")
        ...
        for tid, task in TASK_STATUSES.items():
            if (task.get('type') == 'deploy_application' and
                task.get('status') == 'completed' and
                not task.get('deleted', False)):
        ...
        for app in real_apps:
            ...
            logger.info(f"Added real application: {app['app_name']} from cluster {app['cluster']}")

        # Extract application data from tasks
        applications = []
        for task_id, task in TASK_STATUSES.items():
            if task.get('type') == 'deploy_application' and task.get('status') == 'completed':
                config = task.get('config', {})
                ...
                app = {
                    ...
                    'display_name': config.get('display_name', config.get('app_name', 'Unknown').replace('-', ' ').title()),
        ...
        if include_system_components:
            logger.info(f"include_system_components=True, calling get_system_components()")
            system_apps = get_system_components()
            logger.info(f"get_system_components() returned {len(system_apps)} system components")
...
def get_applications():
    try:
        ...
        for task_id in task_ids:
            ...
            if not task_status: continue # Skip if task somehow disappeared

            logger.debug(f"Processing task {task_id}: {task_status.get('status')}, type: {task_status.get('type')}")

            # Check if this is a COMPLETED deployment task
            # AND that it hasn't been marked as deleted by a subsequent task
            if (task_status.get('type') == 'deploy_application' and
                task_status.get('status') == 'completed' and
                not task_status.get('deleted', False)): # Check for a 'deleted' flag

                config = task_status.get('config', {})
                result = task_status.get('result', {}) # Deployment result might have useful info too

                app_name = config.get('app_name')
                if not app_name:
                    logger.warning(f"Task {task_id} (deploy_application) is completed but missing app_name in config. Skipping.")
                    continue

                cluster_name = config.get('cluster_name')
                namespace = config.get('namespace', 'default')
                deployment_method = config.get('deployment_method', 'kubectl')
                app_version = config.get('app_version', 'N/A') # Use N/A if not specified
                deployment_status_details = result.get('deployment_status', {}) # Get detailed status if available
                metadata = result.get('metadata', {}) # Get metadata if available

                # Ensure deployment_status is a dictionary
                if deployment_status_details is None: deployment_status_details = {}

                # Basic display name generation
                display_name = metadata.get('display_name', app_name.replace('-', ' ').title())
                ...
                logger.debug(f"Added application to list: {application['name']} (ID: {task_id})")
...
async def delete_application(
    ...
    logger.info(f"Received delete request - ID: '{task_id}', appName: '{appName}', clusterName: '{clusterName}'")

    # Find by query parameters first if provided (most reliable method)
    if appName and clusterName:
        logger.info(f"Attempting to find application by name '{appName}' in cluster '{clusterName}'")
        ...
        for tid, task in TASK_STATUSES.items():
            if (task.get('type') == 'deploy_application' and
                ...
                config = task.get('config', {})
                if (config.get('app_name') == appName and
                    config.get('cluster_name') == clusterName):

                    matching_task_id = tid
                    logger.info(f"Found matching task for app '{appName}' in cluster '{clusterName}': {matching_task_id}")
                    break

        if matching_task_id:
            # Use the found task_id instead of the one provided in the URL
            task_id = matching_task_id
            logger.info(f"Using found task_id: {task_id} for app '{appName}' in cluster '{clusterName}'")
    ...
    # Handle numeric IDs for backward compatibility
        ...
        logger.warning(f"Received numeric ID: {task_id}. Attempting to find matching task...")
        ...
        if matching_task_id:
            logger.info(f"Found matching task for numeric ID {task_id}: {matching_task_id}")
            task_id = matching_task_id
        else:
    ...
    if not deployment_task:
        ...
        raise HTTPException(status_code=404, detail=f"Application with ID '{task_id}' not found")

    if deployment_task.get('type') != 'deploy_application':
        logger.error(f"Task with ID '{task_id}' is not a deployment task (type: {deployment_task.get('type')})")
        raise HTTPException(status_code=400, detail=f"ID '{task_id}' does not correspond to a deployment task")
    ...
    logger.info(f"Deleting Kubernetes resources for '{app_name}' in namespace '{namespace}' on cluster '{cluster_name}'")
    ...
    try:
        ...
        logger.info(f"SUCCESS: Deleted application '{app_name}' and marked as deleted in task store")
        ...
        if is_demo:
            ...
            logger.info(f"Added demo app {app_key} to DELETED_DEMO_APPS list - will not be regenerated")
        ...
        logger.error(f"Failed to delete Kubernetes resources for '{app_name}': {str(e)}")
    ...
    try:
        ...
        if action_request.action == "stop":
            ...
            if result.returncode == 0:
                ...
                return create_api_response("success", f"Application {app_name} stopped successfully")
            else:
            ...
            if result.returncode == 0:
                ...
                if task:
                    task['status'] = 'running'
                    task_store.save_task(app_id, task)
                return create_api_response("success", f"Application {app_name} started successfully")
            else:
            ...
            if result.returncode == 0:
                return create_api_response("success", f"Application {app_name} restarted successfully")
            else:
...
async def scale_application(app_id: str, scale_request: ApplicationScale):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return create_api_response("success", f"Application {app_name} scaled to {scale_request.replicas} replicas")
        else:
...
async def get_application_details(app_id: str, cluster_name: str, namespace: str = "default", app_name: str = None):
    """
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for pod in pods_data.get("items", []):
                pod_info = {
                    ...
                    "ready": all(condition.get("status") == "True" for condition in pod.get("status", {}).get("conditions", []) if condition.get("type") == "Ready"),
                    "restart_count": sum(container.get("restartCount", 0) for container in pod.get("status", {}).get("containerStatuses", [])),
        ...
        if result.returncode == 0:
            ...
            for secret in secrets_data.get("items", []):
                secret_info = {
                    ...
                    "data_keys": list(secret.get("data", {}).keys()) if secret.get("data") else []
                }
        ...
        for service in services_info:
            for port in service.get("ports", []):
                ...
                local_port = port_number + 1000 if port_number < 9000 else port_number + 100
        ...
        return create_api_response("success", "Application details retrieved", data={
...
async def get_application_logs(app_id: str, cluster_name: str, namespace: str = "default", lines: int = 100, app_name: str = None):
    """
    ...
    try:
        ...
        if result.returncode == 0:
            return create_api_response("success", "Application logs retrieved", data={
                "logs": result.stdout,
                "app_name": app_name,
                "namespace": namespace,
                "cluster": cluster_name
            ...
            if result_alt.returncode == 0:
                ...
                return create_api_response("warning", "No logs available or application not found", data={
                    "logs": result.stderr or result_alt.stderr or "No logs available",
                    "app_name": app_name,
                    "namespace": namespace,
                    "cluster": cluster_name
                })
...
async def get_application_metrics(app_id: str, cluster_name: str, namespace: str = "default", app_name: str = None):
    """
    ...
    try:
        ...
        return create_api_response("success", "Application metrics retrieved", data={
            "metrics": metrics,
...
async def get_task_statuses():
    try:
        ...
        return {
            ...
            "tasks": task_statuses_serializable
        }
    except Exception as e:
...
async def add_sample_app(cluster_name: str):
    try:
        ...
        logger.info(f"Added sample application {app_name} to TASK_STATUSES with task_id {task_id}")
        return {"status": "success", "message": f"Added sample application {app_name} to TASK_STATUSES", "task_id": task_id}
    except Exception as e:
...
async def set_resource_limits(name: str, limits: ResourceLimits):
    ...
    try:
        ...
        cp_mem = control_plane_config.memory.replace('GB', '') if control_plane_config else '4'
        ...
        script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))),
                             "scripts", "set_resource_limits.sh")
        ...
        if process.returncode != 0:
            logger.error(f"Resource limits script failed with code {process.returncode}")
            return create_api_response(
                "error",
        ...
        # Check control plane
        verify_cmd = ["docker", "inspect", f"{name}-control-plane",
                    "-f", "Memory={{.HostConfig.Memory}},CPUs={{.HostConfig.NanoCpus}}"]
...
def start_api_server(host: str = "0.0.0.0", port: int = 8020, reload: bool = False, environment: str = "dev"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=8020, help="Port to bind the server to")
    parser.add_argument("--mock", action="store_true", help="Use mock data instead of real data")
    parser.add_argument("--task-store", default="file", choices=["memory", "file"], help="Task persistence type")
    parser.add_argument("--task-file", default="./tasks.json", help="Path to task persistence file")
    parser.add_argument("--env", default="dev", choices=["dev", "staging", "prod"], help="Environment to use")
    ...
    logger.info(f"Task persistence: {args.task_store} ({args.task_file if args.task_store == 'file' else ''})")
