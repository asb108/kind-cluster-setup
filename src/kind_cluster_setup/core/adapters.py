...
class KindClusterAdapter:
    ...
    def __init__(
        ...
        if "control_plane_config" in cluster_config:
            ...
            self.config.control_plane_config.memory = control_plane_config.get(
                "memory", "2GB"
            )
    ...
    def create(self) -> None:
        ...
        try:
            ...
            config_path = os.path.join(
                os.path.expanduser("~/.kind-setup/configs"), f"{self.name}.yaml"
            )
            os.makedirs(os.path.dirname(config_path), exist_ok=True)

            # Create the cluster using the KindClient
            self.kind_client.create_cluster(name=self.name, config_file=config_path)
