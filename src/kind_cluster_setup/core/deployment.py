...
class DeploymentStrategy(ABC):
    ...
    @abstractmethod
    def check_status(
        self, app: str, namespace: str, cluster: Cluster
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for manifest_file in manifest_files:
                self.kubectl_client.apply(
                    files=[manifest_file], context=cluster.context, namespace=namespace
                )
    ...
    def check_status(
        ...
        try:
            ...
            pods = self.kubectl_client.get_pods(
                namespace=namespace, context=cluster.context, selector=f"app={app}"
            )
            ...
            if pods:
                all_running = all(
                    pod.get("status", {}).get("phase") == "Running" for pod in pods
                )
...
class HelmDeploymentStrategy(DeploymentStrategy):
    ...
    def deploy(
        ...
        try:
            ...
            logger.info(
                f"Deploying to namespace: {namespace} in cluster: {cluster.name}"
            )
