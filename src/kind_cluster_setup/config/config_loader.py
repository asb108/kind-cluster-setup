...
def load_app_config(
    app: str, environment: str, multi_doc: bool = True
    ...
    if not os.path.exists(config_path):
        ...
        for alt_path in alt_paths:
            ...
            logger.warning(
                f"Could not find config file for {app} in environment {environment}"
            )
            raise FileNotFoundError(
                f"Config file for {app} in environment {environment} not found"
            )
...
def load_cluster_config(environment: str) -> Dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        logger.warning(
            f"Could not find cluster config file for environment {environment}"
        )
